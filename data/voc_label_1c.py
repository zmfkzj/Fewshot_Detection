...
sets=[('_fod', 'train'), ('_fod', 'val'),('_fod', 'test')]

# classes = ["aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow", "diningtable", "dog", "horse", "motorbike", "person", "pottedplant", "sheep", "sofa", "train", "tvmonitor"]
classes = [ 'Bolt', 'Plier', 'Wrench', 'Washer', 'Nail', 'LuggageTag', 'Tag', 'AdjustableClamp', 'Nut', 'BoltNutSet', 'AdjustableWrench', 'MetalSheet', 'Hose', ]
...
def convert_annotation(year, image_id, class_name):
    ...
    out_file = open('VOCdevkit/VOC%s/labels_1c/%s/%s.txt'%(year, class_name, image_id.replace('/','@_')), 'w')
...
for class_name in classes:
    ...
    files = [
        ...
        'voclist/_fod_{}_train.txt'.format(class_name),
        'voclist/_fod_{}_val.txt'.format(class_name),
    ]
    files = ' '.join(files)
